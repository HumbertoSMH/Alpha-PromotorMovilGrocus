...
import mx.com.algroup.promotormovilgrocus.business.Direccion;
...
public class StringUtils {
    ...
    public static String[] armarListaPreciosDesdePrecioBase100( List<Integer> precios ){
        String[] preciosArray = null;
        if( precios == null ){
            preciosArray = new String[1];
            preciosArray[0] = Const.OPCION_PRECIO_OTRO;
            return preciosArray;
        }

        int totalPrecios = precios.size();
        preciosArray = new String[ totalPrecios + 1 ];
        for( int j = 0 ; j < totalPrecios ; j++ ){
            preciosArray[j] = StringUtils.transformarPrecio100APrecioDecimal( "" + ( precios.get(j)  ) );
        }
        preciosArray[ totalPrecios ] = Const.OPCION_PRECIO_OTRO;
        return preciosArray;
    }
